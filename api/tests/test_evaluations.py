...
def setup_entities(client):
    ...
    client.post("/auth/register", json={"email": email, "password": password, "org_id": org_id})
    tok = client.post("/auth/token", data={"username": email, "password": password}, headers={"Content-Type": "application/x-www-form-urlencoded"}).json()["access_token"]
    headers = {"Authorization": f"Bearer {tok}"}
    # Project
    proj = client.post("/api/v1/projects", json={"name": "projE"}, headers=headers).json()
    # Scenario
...
def test_evaluation_happy_path(client, db_session):
    ...
    from app import models
    sc = models.SimulationScenario(project_id=proj["id"], name="s1", config={"distance_to_control_cm": 50, "fg_rgb": [255,255,255], "bg_rgb": [0,0,0], "required_force_N": 15, "capability_N": 20})
    db_session.add(sc)
    db_session.commit()
    ...
    # Create rulepack
    rp_payload = {"name": "pack1", "version": "1.0.0", "rules": {"rules": [{"id": "contrast", "variables": ["contrast_ratio"], "thresholds": {"min_ratio": 4.5}, "condition": "contrast_ratio >= min_ratio", "severity": "high"}]}}
